...
export class FavoriteService {
    ...
    async favorite(id: string, establishmentId: string) {
        ...
        if(user.favorites.indexOf(establishment) !== -1) throw new FavoriteException('Estabelecimento já foi favoritado.')

        this.userRepository.merge(user, { favorites: [...user.favorites, establishment] })
